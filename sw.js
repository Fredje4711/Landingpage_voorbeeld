...
const CACHE_NAME = 'dlml-tools-cache-v1';
...
const urlsToCache = [
  ...
  '/Landingpage_voorbeeld/favicon.ico',      // Het favicon bestand (NIEUW toegevoegd)
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .then(() => {
        console.log('Service Worker: Installatie compleet, bestanden zijn gecached.');
...
self.addEventListener('activate', (event) => {
  ...
  console.log('Service Worker: Activatie compleet');
});
...
self.addEventListener('fetch', (event) => {
  ...
  if (!event.request.url.startsWith(self.location.origin)) {
     console.log('Service Worker: Skipping fetch for external URL:', event.request.url);
  ...
  event.respondWith(
    // 1. Probeer het antwoord uit de cache te halen
        ...
        return fetch(fetchRequest).then(
          (networkResponse) => {
            ...
            if(!networkResponse || networkResponse.status !== 200 || networkResponse.type !== 'basic') {
              console.log('Service Worker: Not caching non-basic or error response from network', event.request.url, networkResponse.status);
            ...
            caches.open(CACHE_NAME)
              .then((cache) => {
                console.log('Service Worker: Putting in cache', event.request.url);
            ...
            console.error('Service Worker: Fetch error, likely network offline.', error);
